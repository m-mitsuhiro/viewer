...
fn generate_video_thumbnail(file_path: &str) -> Result<Vec<u8>, String> {
    ...
    use windows::{
        ...
        Win32::{
            ...
            UI::Shell::{IShellItemImageFactory, SHCreateItemFromParsingName, SIIGBF_BIGGERSIZEOK, SIIGBF_RESIZETOFIT},
        },
    };
    ...
    unsafe {
        ...
        let result = (|| -> Result<Vec<u8>, String> {
            ...
            // PBGRA (premultiplied) → RGBA: undo premultiplication, then swap B↔R
            for chunk in pixels.chunks_mut(4) {
                ...
                if a == 0 {
                    ...
                    // un-premultiply
                    chunk[0] = ((chunk[0] as u32 * 255 + a as u32 / 2) / a as u32).min(255) as u8;
                    chunk[1] = ((chunk[1] as u32 * 255 + a as u32 / 2) / a as u32).min(255) as u8;
                    chunk[2] = ((chunk[2] as u32 * 255 + a as u32 / 2) / a as u32).min(255) as u8;
                }
                chunk.swap(0, 2); // B↔R → RGBA
            }
